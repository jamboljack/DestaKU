...
$(document).ready(function() {
    ...
    tableDataRTRW<?=$no;?> = $('#tableDataRTRW<?=$no;?>').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('member/pengajuan/data_rtrw_list/');?>"+pengajuan_id+"/"+rtrw_grup_id,
            "type": "POST",
        },
        "columnDefs": [
            {
...
function YaData(pengajuan_rtrw_id) {
    ...
    $.ajax({
        url : "<?=site_url('member/pengajuan/pilihya')?>/"+id,
        type: "POST",
...
function TidakData(pengajuan_rtrw_id) {
    var id = pengajuan_rtrw_id;
    $.ajax({
        url : "<?=site_url('member/pengajuan/pilihtidak')?>/"+id,
        type: "POST",
        success: function(data) {
            reload_table_rtrw();
        },
