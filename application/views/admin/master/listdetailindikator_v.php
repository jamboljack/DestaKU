        ...
        <div class="kt-container kt-container--fluid ">
            <div class="kt-subheader__main">
                ...
                <div class="kt-subheader__breadcrumbs">
                    ...
                    <span class="kt-subheader__breadcrumbs-separator"></span>
                    <a href="#" class="kt-subheader__breadcrumbs-link">
                        Indikator : <?=$detail->indikator_nama;?>
                    </a>
                </div>
            </div>
        </div>
            ...
            <div class="kt-portlet__head kt-portlet__head--sm">
                <div class="kt-portlet__head-label">
                    ...
                    <h3 class="kt-portlet__head-title">
                        Daftar Detail Indikator : <?=$detail->indikator_nama;?>
                    </h3>
                </div>
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-actions">
                        <a href="<?=site_url('admin/indikator');?>" class="btn btn-clean"><i class="la la-arrow-left"></i><span class="kt-hidden-mobile">Kembali</span></a>
...
$(document).ready(function() {
    var indikator_id = '<?=$detail->indikator_id;?>';
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/indikator/data_detail_list/');?>"+indikator_id,
            "type": "POST"
        },
        "columnDefs": [
            {
...
function hapusData(indikator_detail_id) {
    var id = indikator_detail_id;
    swal.fire({
        title: 'Anda Yakin ?',
        ...
        if (result.value) {
            $.ajax({
                url : "<?=site_url('admin/indikator/deletedatadetail')?>/"+id,
                type: "POST",
...
$(document).ready(function() {
    $( "#formInput" ).validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/indikator/savedatadetail');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
...
function edit_data(id) {
    $('#formEdit')[0].reset();
    $.ajax({
        url : "<?=site_url('admin/indikator/get_data_detail/');?>"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('#id').val(data.indikator_detail_id);
            $('#indikator_detail_nama').val(data.indikator_detail_nama);
            $('#lstStatus').val(data.indikator_detail_status);
...
$(document).ready(function() {
    $( "#formEdit" ).validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/indikator/updatedatadetail');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
...
<div class="modal fade" id="formModalAdd" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <input type="hidden" name="indikator_id" value="<?=$detail->indikator_id;?>">
        ...
        <div class="modal-content">
            ...
            <input type="hidden" name="id" id="id">
                <div class="modal-body">
                    <div class="form-group">
                        <label>Keterangan</label>
                        <input type="text" class="form-control" placeholder="Input Keterangan" name="nama" id="indikator_detail_nama" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <label>Status</label>
