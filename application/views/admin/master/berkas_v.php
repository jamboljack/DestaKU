...
$(document).ready(function() {
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/berkas/data_list');?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
function hapusData(berkas_id) {
    ...
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url : "<?=site_url('admin/berkas/deletedata')?>/"+id,
                type: "POST",
    ...
    $( "#formInput" ).validate({
        rules: { 
            nama: {
                required: true,
                remote: {
                    url: "<?=site_url('admin/berkas/register_nama_exists'); ?>",
                    type: "post",
                    data: {
                        nama: function() { 
                            return $("#nama").val(); 
        ...
        messages: { 
            nama: { required :'Nama Berkas required', remote:'Nama Berkas sudah Ada' }
        },
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/berkas/savedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
...
function edit_data(id) {
    $('#formEdit')[0].reset();
    $.ajax({
        url : "<?=site_url('admin/berkas/get_data/');?>"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
...
$(document).ready(function() {
    $( "#formEdit" ).validate({
        ...
        messages: { 
            nama: { required :'Nama Berkas required' }
        },
        invalidHandler: function(event, validator) {
            KTUtil.scrollTop();
        },
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/berkas/updatedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
            ...
            <form class="kt-form" method="post" name="formInput" id="formInput">
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Input Nama Berkas" name="nama" id="nama" autocomplete="off">
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <input type="hidden" name="id" id="id">
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama Berkas</label>
                        <input type="text" class="form-control" placeholder="Input Nama Berkas" name="nama" id="berkas_nama" autocomplete="off">
                    </div>
                </div>
                <div class="modal-footer">
