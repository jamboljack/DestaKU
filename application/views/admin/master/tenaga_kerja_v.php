...
$(document).ready(function() {
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/tenaga_kerja/data_list');?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
function hapusData(tenaga_kerja_id) {
    var id = tenaga_kerja_id;
    swal.fire({
        title: 'Anda Yakin ?',
        ...
        if (result.value) {
            $.ajax({
                url : "<?=site_url('admin/tenaga_kerja/deletedata')?>/"+id,
                type: "POST",
    ...
    $( "#formInput" ).validate({
        rules: { 
            nama: {
                required: true,
                remote: {
                    url: "<?=site_url('admin/tenaga_kerja/register_nama_exists'); ?>",
                    type: "post",
                    data: {
                        nama: function() { 
                            return $("#nama").val(); 
        ...
        messages: { 
            nama: { required :'Nama Tenaga Kerja required', remote:'Nama Tenaga Kerja sudah Ada' }
        },
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/tenaga_kerja/savedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
...
function edit_data(id) {
    $('#formEdit')[0].reset();
    $.ajax({
        url : "<?=site_url('admin/tenaga_kerja/get_data/');?>"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('#id').val(data.tenaga_kerja_id);
            $('#tenaga_kerja_nama').val(data.tenaga_kerja_nama);
...
$(document).ready(function() {
    $( "#formEdit" ).validate({
        ...
        messages: { 
            nama: { required :'Nama Tenaga Kerja required' }
        },
        invalidHandler: function(event, validator) {
            KTUtil.scrollTop();
        },
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/tenaga_kerja/updatedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
            ...
            <form class="kt-form" method="post" name="formInput" id="formInput">
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Input Nama Tenaga Kerja" name="nama" id="nama" autocomplete="off">
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <input type="hidden" name="id" id="id">
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama Tenaga Kerja</label>
                        <input type="text" class="form-control" placeholder="Input Nama Tenaga Kerja" name="nama" id="tenaga_kerja_nama" autocomplete="off">
                    </div>
                </div>
                <div class="modal-footer">
