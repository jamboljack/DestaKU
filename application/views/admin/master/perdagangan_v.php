...
$(document).ready(function() {
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/perdagangan/data_list');?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
function hapusData(perdagangan_id) {
    var id = perdagangan_id;
    swal.fire({
        title: 'Anda Yakin ?',
        ...
        if (result.value) {
            $.ajax({
                url : "<?=site_url('admin/perdagangan/deletedata')?>/"+id,
                type: "POST",
    ...
    $( "#formInput" ).validate({
        rules: { 
            nama: {
                required: true,
                remote: {
                    url: "<?=site_url('admin/perdagangan/register_nama_exists'); ?>",
                    type: "post",
                    data: {
                        nama: function() { 
                            return $("#nama").val(); 
        ...
        messages: { 
            nama: { required :'Nama Perdagangan required', remote:'Nama Perdagangan sudah Ada' }
        },
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/perdagangan/savedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
...
function edit_data(id) {
    $('#formEdit')[0].reset();
    $.ajax({
        url : "<?=site_url('admin/perdagangan/get_data/');?>"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('#id').val(data.perdagangan_id);
            $('#perdagangan_nama').val(data.perdagangan_nama);
...
$(document).ready(function() {
    $( "#formEdit" ).validate({
        ...
        messages: { 
            nama: { required :'Nama Perdagangan required' }
        },
        invalidHandler: function(event, validator) {
            KTUtil.scrollTop();
        },
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/perdagangan/updatedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
            ...
            <form class="kt-form" method="post" name="formInput" id="formInput">
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama Perdagangan</label>
                        <input type="text" class="form-control" placeholder="Input Nama Perdagangan" name="nama" id="nama" autocomplete="off">
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <input type="hidden" name="id" id="id">
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama Perdagangan</label>
                        <input type="text" class="form-control" placeholder="Input Nama Perdagangan" name="nama" id="perdagangan_nama" autocomplete="off">
                    </div>
                </div>
                <div class="modal-footer">
