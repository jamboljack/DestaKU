...
$(document).ready(function() {
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/mitigasi/data_list');?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
function hapusData(mitigasi_id) {
    var id = mitigasi_id;
    swal.fire({
        title: 'Anda Yakin ?',
        ...
        if (result.value) {
            $.ajax({
                url : "<?=site_url('admin/mitigasi/deletedata')?>/"+id,
                type: "POST",
    ...
    $( "#formInput" ).validate({
        rules: { 
            nama: {
                required: true,
                remote: {
                    url: "<?=site_url('admin/mitigasi/register_nama_exists'); ?>",
                    type: "post",
                    data: {
                        nama: function() { 
                            return $("#nama").val(); 
        ...
        messages: { 
            nama: { required :'Nama Mitigasi required', remote:'Nama Mitigasi sudah Ada' }
        },
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/mitigasi/savedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
...
function edit_data(id) {
    $('#formEdit')[0].reset();
    $.ajax({
        url : "<?=site_url('admin/mitigasi/get_data/');?>"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('#id').val(data.mitigasi_id);
            $('#mitigasi_nama').val(data.mitigasi_nama);
...
$(document).ready(function() {
    $( "#formEdit" ).validate({
        ...
        messages: { 
            nama: { required :'Nama Mitigasi required' }
        },
        invalidHandler: function(event, validator) {
            KTUtil.scrollTop();
        },
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/mitigasi/updatedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
            ...
            <form class="kt-form" method="post" name="formInput" id="formInput">
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama Mitigasi</label>
                        <input type="text" class="form-control" placeholder="Input Nama Mitigasi" name="nama" id="nama" autocomplete="off">
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <input type="hidden" name="id" id="id">
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama Mitigasi</label>
                        <input type="text" class="form-control" placeholder="Input Nama Mitigasi" name="nama" id="mitigasi_nama" autocomplete="off">
                    </div>
                </div>
                <div class="modal-footer">
