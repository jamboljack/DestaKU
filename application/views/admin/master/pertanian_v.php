...
$(document).ready(function() {
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('admin/pertanian/data_list');?>",
            "type": "POST"
        },
        "columnDefs": [
            {
...
function hapusData(pertanian_id) {
    var id = pertanian_id;
    swal.fire({
        title: 'Anda Yakin ?',
        ...
        if (result.value) {
            $.ajax({
                url : "<?=site_url('admin/pertanian/deletedata')?>/"+id,
                type: "POST",
    ...
    $( "#formInput" ).validate({
        rules: { 
            nama: {
                required: true,
                remote: {
                    url: "<?=site_url('admin/pertanian/register_nama_exists'); ?>",
                    type: "post",
                    data: {
                        nama: function() { 
                            return $("#nama").val(); 
        ...
        messages: { 
            nama: { required :'Nama Pertanian required', remote:'Nama Pertanian sudah Ada' }
        },
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/pertanian/savedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
...
function edit_data(id) {
    $('#formEdit')[0].reset();
    $.ajax({
        url : "<?=site_url('admin/pertanian/get_data/');?>"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('#id').val(data.pertanian_id);
            $('#pertanian_nama').val(data.pertanian_nama);
...
$(document).ready(function() {
    $( "#formEdit" ).validate({
        ...
        messages: { 
            nama: { required :'Nama Pertanian required' }
        },
        invalidHandler: function(event, validator) {
            KTUtil.scrollTop();
        },
        submitHandler: function (form) {
            ...
            $.ajax({
                url: '<?=site_url('admin/pertanian/updatedata');?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal.fire({
            ...
            <form class="kt-form" method="post" name="formInput" id="formInput">
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="Input Nama Pertanian" name="nama" id="nama" autocomplete="off">
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <input type="hidden" name="id" id="id">
                <div class="modal-body">
                    <div class="form-group">
                        <label>Nama Pertanian</label>
                        <input type="text" class="form-control" placeholder="Input Nama Pertanian" name="nama" id="pertanian_nama" autocomplete="off">
                    </div>
                </div>
                <div class="modal-footer">
